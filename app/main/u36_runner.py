# -*- coding: utf-8 -*-
from __future__ import absolute_import, print_function, unicode_literals

import io
import os

try:
    import yaml
except ImportError as exc:
    yaml = None
    _yaml_import_error = exc
else:
    _yaml_import_error = None

from app.compat import to_text
from app.collectors.network_service_reader import NetworkServiceReader
from app.models.check_result import CheckResult


class U36Runner(object):
    """
    U-36 r 계열 서비스 비활성화 점검 실행기

    주석/설명: 한국어
    사용자 출력: 영어
    Python 2.7 ~ 3.x 호환

    기능:
    - /etc/inetd.conf에서 활성 r 서비스 엔트리 확인
    - /etc/xinetd.d/rsh, rlogin, rexec에서 disable 값 확인
    - systemd service/socket active/enabled 상태 확인
    - /etc/hosts.equiv, /root/.rhosts, /home/*/.rhosts 탐지
    - 활성 서비스 또는 신뢰 파일이 있으면 FAIL

    차별점:
    - 단순 서비스 상태만 보는 것이 아니라 inetd/xinetd/systemd/trust file을 함께 본다.
    - r 계열 서비스는 설정 파일이 없는 것이 일반적으로 안전한 상태일 수 있으므로 missing optional로 별도 수집한다.
    """

    def __init__(self, check_dir=None):
        self.app_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self.check_dir = check_dir or os.path.join(
            self.app_dir,
            "checks",
            "u36_r_services_disabled"
        )

        self.reader = NetworkServiceReader()

        self.metadata = {}
        self.targets = {}
        self.policy = {}
        self.messages = {}

    def run(self):
        try:
            self._load_configs()
        except Exception as exc:
            return self._build_error_result(
                "Configuration loading failed: {0}".format(to_text(exc))
            )

        result = CheckResult(
            code=self.metadata.get("code", "U-36"),
            name=self.metadata.get("name", "Disable r Services"),
            severity=self.metadata.get("severity", "high"),
            category=self.metadata.get("category", "service_management"),
            status="MANUAL",
            success=True,
            summary=self._get_message(
                "manual",
                "summary",
                default="r service status requires manual verification."
            ),
            detail=self._get_message(
                "manual",
                "detail",
                default="Some r service configuration files or service states could not be fully verified."
            ),
            requires_root=self.metadata.get("requires_root", "partial"),
            remediation_summary=self._get_remediation_summary(),
            remediation_steps=self._get_remediation_steps()
        )

        files = self._collect_files()
        result.raw["files"] = self._serialize_file_map(files)

        inetd_info = self._check_inetd(files)
        xinetd_info = self._check_xinetd(files)
        systemd_info = self._check_systemd()
        trust_info = self._check_trust_files(files)

        result.raw["inetd_info"] = inetd_info
        result.raw["xinetd_info"] = xinetd_info
        result.raw["systemd_info"] = systemd_info
        result.raw["trust_info"] = trust_info

        active_r_services = []
        active_r_services.extend(inetd_info.get("active_services", []))
        active_r_services.extend(xinetd_info.get("active_services", []))
        active_r_services.extend(systemd_info.get("active_services", []))
        active_r_services = self._dedupe_keep_order(active_r_services)

        trust_files_detected = []
        trust_files_detected.extend(trust_info.get("system_trust_files", []))
        trust_files_detected.extend(trust_info.get("user_trust_files", []))

        missing_optional_paths = self._collect_missing_optional_paths(files)
        missing_optional_paths.extend(trust_info.get("missing_roots", []))
        missing_optional_paths = self._dedupe_keep_order(missing_optional_paths)

        manual_reasons = []
        manual_reasons.extend(xinetd_info.get("manual_reasons", []))
        manual_reasons.extend(systemd_info.get("manual_reasons", []))

        result.add_evidence(
            key="inetd_r_service_entries",
            label=self._label("inetd_r_service_entries", "r service entries in inetd.conf"),
            source="/etc/inetd.conf",
            value=inetd_info.get("active_entries", []) if inetd_info.get("active_entries", []) else ["(none)"],
            status="fail" if inetd_info.get("active_entries") else "ok"
        )

        result.add_evidence(
            key="xinetd_r_service_entries",
            label=self._label("xinetd_r_service_entries", "r service entries in xinetd configuration"),
            source="/etc/xinetd.d/*",
            value=xinetd_info.get("items", []),
            status="fail" if xinetd_info.get("active_services") else (
                "manual" if xinetd_info.get("manual_reasons") else "ok"
            )
        )

        result.add_evidence(
            key="systemd_r_service_status",
            label=self._label("systemd_r_service_status", "r service status from systemd"),
            source="systemd",
            value=systemd_info.get("items", []),
            status="fail" if systemd_info.get("active_services") else (
                "manual" if systemd_info.get("manual_reasons") else "ok"
            )
        )

        result.add_evidence(
            key="hosts_equiv_exists",
            label=self._label("hosts_equiv_exists", "/etc/hosts.equiv exists"),
            source="/etc/hosts.equiv",
            value=trust_info.get("hosts_equiv_exists", False),
            status="fail" if trust_info.get("hosts_equiv_exists", False) else "ok"
        )

        result.add_evidence(
            key="rhosts_files",
            label=self._label("rhosts_files", ".rhosts trust files"),
            source="/home/*/.rhosts, /root/.rhosts",
            value=trust_info.get("user_trust_files", []) if trust_info.get("user_trust_files", []) else ["(none)"],
            status="fail" if trust_info.get("user_trust_files") else "ok"
        )

        result.add_evidence(
            key="active_r_services",
            label=self._label("active_r_services", "Active r services"),
            source="inetd, xinetd, systemd",
            value=active_r_services if active_r_services else ["(none)"],
            status="fail" if active_r_services else "ok"
        )

        result.add_evidence(
            key="trust_files_detected",
            label=self._label("trust_files_detected", "Detected r service trust files"),
            source="/etc/hosts.equiv, .rhosts",
            value=trust_files_detected if trust_files_detected else ["(none)"],
            status="fail" if trust_files_detected else "ok"
        )

        result.add_evidence(
            key="missing_optional_paths",
            label=self._label("missing_optional_paths", "Missing optional paths"),
            source="r service related paths",
            value=missing_optional_paths if missing_optional_paths else ["(none)"],
            status="manual" if missing_optional_paths else "ok",
            notes="Missing inetd/xinetd/r service files are often normal on modern Linux systems."
        )

        reasons = []

        if active_r_services:
            reasons.append(
                "Active r service configuration or service state was detected: {0}".format(
                    ", ".join(active_r_services)
                )
            )

        if trust_files_detected:
            reasons.append(
                "r service trust files were detected."
            )

        if manual_reasons:
            reasons.extend(manual_reasons)

        if missing_optional_paths:
            reasons.append(
                "Some optional r service paths are missing, which may be normal depending on the distribution."
            )

        if active_r_services or trust_files_detected:
            result.set_status("FAIL", success=False)
            result.summary = self._get_message(
                "fail",
                "summary",
                default="r services or trust files were detected."
            )
            result.detail = self._merge_detail(
                self._get_message(
                    "fail",
                    "detail",
                    default="Legacy r services use weak trust-based authentication and may allow account compromise or unauthorized remote access."
                ),
                reasons
            )
            return result

        if manual_reasons:
            result.set_status("MANUAL", success=True)
            result.summary = self._get_message(
                "manual",
                "summary",
                default="r service status requires manual verification."
            )
            result.detail = self._merge_detail(
                self._get_message(
                    "manual",
                    "detail",
                    default="Some r service configuration files or service states could not be fully verified."
                ),
                reasons
            )
            return result

        result.set_status("PASS", success=True)
        result.summary = self._get_message(
            "pass",
            "summary",
            default="r services are disabled."
        )
        result.detail = self._merge_detail(
            self._get_message(
                "pass",
                "detail",
                default="rsh, rlogin, and rexec services are not active, and no r service trust files were detected."
            ),
            ["No active r service or r service trust file was detected."]
        )
        return result

    def _collect_files(self):
        collected = {}

        for item in self.targets.get("files", {}).get("optional", []):
            path = to_text(item.get("path", "")).strip()
            if not path:
                continue

            collected[path] = self.reader.read_file(path)

        return collected

    def _check_inetd(self, files):
        rule = self.policy.get("rules", {}).get("inetd_rule", {})
        service_names = rule.get("service_names", [])

        path = "/etc/inetd.conf"
        file_result = files.get(path)

        if not self.reader.file_exists(file_result):
            return {
                "exists": False,
                "active_entries": [],
                "active_services": [],
            }

        if not file_result.success or not file_result.content:
            return {
                "exists": True,
                "active_entries": [],
                "active_services": [],
                "manual_reasons": ["inetd.conf exists but could not be fully read."],
            }

        parsed = self.reader.parse_inetd_r_services(
            file_result.content,
            service_names=service_names
        )

        active_services = []
        for service in parsed.get("matched_services", []):
            active_services.append("inetd:{0}".format(service))

        return {
            "exists": True,
            "active_entries": parsed.get("active_entries", []),
            "active_services": active_services,
        }

    def _check_xinetd(self, files):
        rule = self.policy.get("rules", {}).get("xinetd_rule", {})
        service_files = rule.get("service_files", [])
        fail_values = self._normalize_lower_list(rule.get("fail_values", ["no"]))

        items = []
        active_services = []
        manual_reasons = []

        for path in service_files:
            path_text = to_text(path).strip()
            file_result = files.get(path_text)

            if not self.reader.file_exists(file_result):
                items.append({
                    "path": path_text,
                    "exists": False,
                    "disable_value": "(missing)",
                    "status": "not-found",
                })
                continue

            service_name = os.path.basename(path_text)

            if not file_result.success or not file_result.content:
                items.append({
                    "path": path_text,
                    "exists": True,
                    "disable_value": "(unreadable)",
                    "status": "manual",
                })
                manual_reasons.append(
                    "{0} exists but could not be fully read.".format(path_text)
                )
                continue

            parsed = self.reader.parse_xinetd_r_service(
                file_result.content,
                service_name=service_name
            )

            disable_value = to_text(parsed.get("disable_value", "")).lower()

            status = "ok"
            if disable_value in fail_values:
                status = "active"
                active_services.append("xinetd:{0}".format(service_name))
            elif not disable_value:
                status = "manual"
                manual_reasons.append(
                    "{0} does not contain a clear disable value.".format(path_text)
                )

            items.append({
                "path": path_text,
                "exists": True,
                "service_name": service_name,
                "disable_value": disable_value if disable_value else "(not detected)",
                "matched_lines": parsed.get("matched_lines", []),
                "status": status,
            })

        return {
            "items": items,
            "active_services": active_services,
            "manual_reasons": manual_reasons,
        }

    def _check_systemd(self):
        services = self.targets.get("services", [])

        items = []
        active_services = []
        manual_reasons = []

        for service in services:
            name = to_text(service.get("name", "")).strip()
            aliases = service.get("aliases", [])

            if not name:
                continue

            inspected = self.reader.inspect_systemd_service(name, aliases=aliases)
            items.append(inspected)

            if inspected.get("active") or inspected.get("enabled"):
                active_services.append("systemd:{0}".format(name))

            # systemctl이 동작하지 않는 환경이면 unknown이 많을 수 있으므로 수동 근거로 남긴다.
            unknown_count = 0
            for unit in inspected.get("units", []):
                if unit.get("active_state") == "unknown" and unit.get("enabled_state") == "unknown":
                    unknown_count += 1

            if unknown_count == len(inspected.get("units", [])) and inspected.get("units", []):
                manual_reasons.append(
                    "systemd state for {0} could not be fully verified.".format(name)
                )

        return {
            "items": items,
            "active_services": active_services,
            "manual_reasons": manual_reasons,
        }

    def _check_trust_files(self, files):
        hosts_equiv_path = "/etc/hosts.equiv"
        hosts_equiv_file = files.get(hosts_equiv_path)

        system_trust_files = []
        hosts_equiv_exists = False

        if self.reader.file_exists(hosts_equiv_file):
            hosts_equiv_exists = True
            system_trust_files.append({
                "path": hosts_equiv_path,
                "type": "hosts.equiv",
            })

        search_config = self.targets.get("search", {}).get("trust_files", {})
        roots = search_config.get("roots", ["/home", "/root"])
        names = search_config.get("names", [".rhosts", "hosts.equiv"])

        found_info = self.reader.find_trust_files(
            roots=roots,
            names=names
        )

        user_trust_files = []
        for item in found_info.get("found", []):
            path = to_text(item.get("path", ""))
            if path == hosts_equiv_path:
                continue
            user_trust_files.append(item)

        return {
            "hosts_equiv_exists": hosts_equiv_exists,
            "system_trust_files": system_trust_files,
            "user_trust_files": user_trust_files,
            "missing_roots": found_info.get("missing_roots", []),
            "warnings": found_info.get("warnings", []),
        }

    @staticmethod
    def _serialize_file_map(file_map):
        result = {}

        for path, file_result in file_map.items():
            result[path] = file_result.to_dict() if file_result else None

        return result

    def _collect_missing_optional_paths(self, files):
        missing = []

        for path, file_result in files.items():
            if not self.reader.file_exists(file_result):
                missing.append(path)

        return missing

    @staticmethod
    def _normalize_lower_list(values):
        result = []

        for item in values or []:
            text = to_text(item).strip().lower()
            if text:
                result.append(text)

        return result

    @staticmethod
    def _dedupe_keep_order(items):
        seen = set()
        result = []

        for item in items or []:
            if item is None:
                continue

            normalized = to_text(item).strip()
            if not normalized:
                continue

            if normalized not in seen:
                seen.add(normalized)
                result.append(normalized)

        return result

    def _load_configs(self):
        if yaml is None:
            raise RuntimeError(
                "PyYAML is required. Please install it first. Cause: {0}".format(
                    to_text(_yaml_import_error)
                )
            )

        self.metadata = self._load_yaml(os.path.join(self.check_dir, "metadata.yaml"))
        self.targets = self._load_yaml(os.path.join(self.check_dir, "targets.yaml"))
        self.policy = self._load_yaml(os.path.join(self.check_dir, "policy.yaml"))
        self.messages = self._load_yaml(os.path.join(self.check_dir, "messages.yaml"))

    @staticmethod
    def _load_yaml(path):
        if not os.path.exists(path):
            raise IOError("Configuration file not found: {0}".format(path))

        with io.open(path, "r", encoding="utf-8") as f:
            data = yaml.safe_load(f) or {}

        if not isinstance(data, dict):
            raise ValueError("YAML root must be a dict: {0}".format(path))

        return data

    def _get_message(self, status, field, default=""):
        direct = self.messages.get(status, {})

        if isinstance(direct, dict) and field in direct:
            return to_text(direct.get(field, default))

        reverse = self.messages.get(field, {})

        if isinstance(reverse, dict) and status in reverse:
            return to_text(reverse.get(status, default))

        return to_text(default)

    def _get_remediation_summary(self):
        remediation = self.messages.get("remediation", {})

        if isinstance(remediation, dict):
            return to_text(
                remediation.get(
                    "summary",
                    "Disable r services and remove unnecessary r service trust files."
                )
            )

        return "Disable r services and remove unnecessary r service trust files."

    def _get_remediation_steps(self):
        remediation = self.messages.get("remediation", {})

        if isinstance(remediation, dict):
            steps = remediation.get("steps", remediation.get("actions", []))
            return self._dedupe_keep_order(steps)

        return []

    def _label(self, key, default):
        labels = self.messages.get("evidence_labels", {})

        if isinstance(labels, dict):
            return to_text(labels.get(key, default))

        return to_text(default)

    @staticmethod
    def _merge_detail(base_detail, reasons):
        filtered = []

        for reason in reasons or []:
            normalized = to_text(reason).strip()

            if normalized:
                filtered.append(normalized)

        if not filtered:
            return to_text(base_detail).strip()

        merged = [to_text(base_detail).strip(), "", "Decision reasons:"]

        for reason in filtered:
            merged.append("- {0}".format(reason))

        return "\n".join(merged)

    def _build_error_result(self, message):
        result = CheckResult(
            code="U-36",
            name="Disable r Services",
            severity="high",
            category="service_management",
            status="ERROR",
            success=False,
            summary="An error occurred while running the check.",
            detail=to_text(message),
            requires_root="partial"
        )
        result.add_error(to_text(message))
        return result