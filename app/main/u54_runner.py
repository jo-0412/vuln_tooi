...
class U54Runner(object):
    """
    U-54 암호화되지 않는 FTP 서비스 비활성화 점검 실행기
    ...
    기능:
    - /etc/inetd.conf 에서 FTP 활성 엔트리 확인
    - /etc/xinetd.d/ftp 에서 disable 값 확인
    - systemd FTP 관련 service/socket active/enabled 상태 확인
    - vsftpd, proftpd, pure-ftpd 등 FTP 설정 파일 존재 여부 확인
    - FTP가 inetd/xinetd/systemd 중 하나라도 활성화되어 있으면 FAIL
    - FTP 설정 파일은 존재하지만 서비스 활성 여부가 없으면 MANUAL 근거로 남김

    차별점:
    - U-52 Telnet 점검과 같은 network_service_reader.py를 재사용한다.
    - U-52는 telnet 하나를 중심으로 보지만,
      U-54는 ftp, ftpd, vsftpd, proftpd, pure-ftpd 계열을 함께 확인한다.
    - SFTP는 SSH 기반이므로 일반 FTP로 보지 않는다.
    - FTPS는 암호화 FTP일 수 있지만, 암호화 강제 여부는 별도 확인이 필요하므로 MANUAL 근거로 남긴다.
    """
    ...
    def run(self):
        ...
        result = CheckResult(
            code=self.metadata.get("code", "U-54"),
            name=self.metadata.get("name", "Disable Unencrypted FTP Service"),
            ...
            summary=self._get_message(
                "manual",
                "summary",
                default="FTP service status requires manual verification."
            ),
            detail=self._get_message(
                "manual",
                "detail",
                default="Some FTP-related configuration files or service states could not be fully verified."
            ),
        ...
        ftp_config_info = self._check_ftp_config_files(files)

        result.raw["inetd_info"] = inetd_info
        result.raw["xinetd_info"] = xinetd_info
        result.raw["systemd_info"] = systemd_info
        result.raw["ftp_config_info"] = ftp_config_info

        active_ftp_services = []
        active_ftp_services.extend(inetd_info.get("active_services", []))
        active_ftp_services.extend(xinetd_info.get("active_services", []))
        active_ftp_services.extend(systemd_info.get("active_services", []))
        active_ftp_services = self._dedupe_keep_order(active_ftp_services)
        ...
        if ftp_config_info.get("config_files"):
            manual_reasons.append(
                "FTP daemon configuration files exist and encryption enforcement requires manual verification."
            )

        # 1. inetd 증적
        result.add_evidence(
            key="inetd_ftp_entries",
            label=self._label("inetd_ftp_entries", "FTP entries in inetd.conf"),
        ...
        # 2. xinetd 증적
        result.add_evidence(
            key="xinetd_ftp_entries",
            label=self._label("xinetd_ftp_entries", "FTP entries in xinetd configuration"),
            source="/etc/xinetd.d/ftp",
        ...
        # 3. systemd 증적
        result.add_evidence(
            key="systemd_ftp_status",
            label=self._label("systemd_ftp_status", "FTP service status from systemd"),
            source="systemd",
        ...
        # 4. FTP 설정 파일 증적
        result.add_evidence(
            key="ftp_config_files",
            label=self._label("ftp_config_files", "FTP daemon configuration files"),
            source="FTP daemon configuration files",
            value=ftp_config_info.get("config_files", []) if ftp_config_info.get("config_files", []) else ["(none)"],
            status="manual" if ftp_config_info.get("config_files") else "ok",
            notes="If FTP configuration files exist, verify whether plaintext login is disabled or encryption is enforced."
        )

        # 5. 활성 FTP 서비스 증적
        result.add_evidence(
            key="active_ftp_services",
            label=self._label("active_ftp_services", "Active FTP services"),
            source="inetd, xinetd, systemd",
            value=active_ftp_services if active_ftp_services else ["(none)"],
            status="fail" if active_ftp_services else "ok"
        )
        ...
        result.add_evidence(
            ...
            source="FTP related paths",
            value=missing_optional_paths if missing_optional_paths else ["(none)"],
            status="manual" if missing_optional_paths else "ok",
            notes="Missing FTP configuration files are usually normal on modern Linux systems."
        )

        # 7. 대체 프로토콜 확인 증적
        result.add_evidence(
            key="secure_replacement_note",
            label=self._label("secure_replacement_note", "SFTP/FTPS replacement verification"),
            source="manual",
            value="Verify that SFTP or FTPS is used instead of plain FTP if file transfer is required.",
            status="manual"
        )

        reasons = []

        if active_ftp_services:
            reasons.append(
                "Active FTP configuration or service state was detected: {0}".format(
                    ", ".join(active_ftp_services)
                )
            )

        if ftp_config_info.get("config_files"):
            reasons.append(
                "FTP daemon configuration files were detected and require encryption enforcement review."
            )
        ...
        if missing_optional_paths:
            reasons.append(
                "Some optional FTP-related paths are missing, which is usually normal on modern Linux systems."
            )

        reasons.append(
            "SFTP or FTPS replacement usage should be verified separately if file transfer is required."
        )

        # FTP가 하나라도 활성화되어 있으면 FAIL
        if active_ftp_services:
            result.set_status("FAIL", success=False)
            result.summary = self._get_message(
                "fail",
                "summary",
                default="Unencrypted FTP service is enabled or running."
            )
            result.detail = self._merge_detail(
                self._get_message(
                    "fail",
                    "detail",
                    default="Plain FTP transmits account credentials and file data without encryption, making them vulnerable to network sniffing and credential theft."
                ),
                reasons
            )
            return result

        # FTP 설정 파일이 존재하면 실제 암호화 강제 여부를 자동 단정하지 않는다.
        if ftp_config_info.get("config_files"):
            ...
            result.detail = self._merge_detail(
                self._get_message(
                    "manual",
                    "detail",
                    default="FTP configuration files exist and encryption enforcement requires manual confirmation."
                ),
                reasons
            )
            return result
        ...
        result.summary = self._get_message(
            "pass",
            "summary",
            default="Unencrypted FTP service is disabled."
        )
        result.detail = self._merge_detail(
            self._get_message(
                "pass",
                "detail",
                default="Plain FTP is not active in inetd, xinetd, or systemd configuration."
            ),
            ["No active FTP service or socket was detected."]
        )
        return result
    ...
    def _check_inetd(self, files):
        """
        /etc/inetd.conf에서 FTP 활성 라인을 확인한다.
        """
        rule = self.policy.get("rules", {}).get("inetd_rule", {})
        service_names = rule.get("service_names", ["ftp", "ftpd"])
        ...
        # service_names를 받기 때문에 FTP 탐지에도 재사용 가능하다.
    ...
    def _check_xinetd(self, files):
        """
        /etc/xinetd.d/ftp에서 disable 값을 확인한다.
        """
        rule = self.policy.get("rules", {}).get("xinetd_rule", {})
        target_file = to_text(rule.get("target_file", "/etc/xinetd.d/ftp")).strip()
        service_name = to_text(rule.get("service_name", "ftp")).strip()
    ...
    def _check_systemd(self):
        """
        systemd에서 FTP 관련 service/socket 상태를 확인한다.
        """
    ...
    def _check_ftp_config_files(self, files):
        """
        FTP 데몬 설정 파일 존재 여부를 확인한다.

        주의:
        - 설정 파일이 있다는 것만으로 FTP가 활성화되었다고 단정하지 않는다.
        - 다만 암호화 강제 여부는 별도 검토가 필요하므로 MANUAL 근거로 사용한다.
        """
        rule = self.policy.get("rules", {}).get("ftp_config_rule", {})
        ...
        for path in config_files:
            ...
            if self.reader.file_exists(file_result):
                item = {
                    "path": path_text,
                    "exists": True,
                    "readable": bool(file_result and file_result.success),
                }

                if file_result and file_result.success and file_result.content:
                    item["preview"] = self._first_non_empty_line(file_result.content)
                else:
                    item["preview"] = ""
    ...
    @staticmethod
    def _first_non_empty_line(content):
        """
        설정 파일 preview용으로 첫 번째 활성 라인만 가져온다.
        """
    ...
    def _get_remediation_summary(self):
        ...
        if isinstance(remediation, dict):
            return to_text(
                remediation.get(
                    "summary",
                    "Disable unencrypted FTP and use SFTP or FTPS for secure file transfer."
                )
            )

        return "Disable unencrypted FTP and use SFTP or FTPS for secure file transfer."
    ...
    def _build_error_result(self, message):
        result = CheckResult(
            code="U-54",
            name="Disable Unencrypted FTP Service",
            severity="high",
